        ...
        if (db)
        {
            foreach (const QHostAddress &address, QNetworkInterface::allAddresses()) {
                if (address.protocol() == QAbstractSocket::IPv4Protocol && address != QHostAddress(QHostAddress::LocalHost)){
                     ...
                     const char* code = GeoIP_country_code_by_ipnum(db, address.toIPv4Address());
                     if (code){
                         selfCode = code;
                         break;
                     }
            ...
            qDebug("geo ip database is null");
        }

        GeoIP_delete(db);
    }

    qDebug() << "self code %s" << selfCode;
  } else {
